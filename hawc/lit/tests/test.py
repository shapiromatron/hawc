    ...
    def test_clean_import_string(self):
        ...
        for search_string in failed_strings:
            ...
            response = self.client.post(
                reverse("lit:import_new", kwargs={"pk": self.assessment_pk}), self.data
            )
        ...
        self.assertFormError(response, "form", "search_string", "IDs must be unique.")
...
class PubMedTest(TestCase):
    ...
    def test_search(self):
        ...
        url = reverse("lit:search_query", kwargs={"pk": self.assessment_pk, "slug": search.slug})
        ...
        self.assertEqual(models.Reference.objects.filter(identifiers__in=i_pks).count(), i_count)
...
class HEROTest(TestCase):
    ...
    def test_existing_pubmed_hero_add(self):
        ...
        url_run_query = reverse(
            "lit:search_query", kwargs={"pk": self.assessment_pk, "slug": self.pm_data["slug"]},
        )
