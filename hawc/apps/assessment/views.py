...
class AssessmentPublicList(ListView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['metadata'] = [
            ...
            f"Imported study data has been used to create {apps.get_model('summary', 'Visual').objects.count() + apps.get_model('summary', 'DataPivot').objects.count()} custom data visualizations",
        ]
        if self.request.user.is_authenticated:
            context["breadcrumbs"] = Breadcrumb.build_crumbs(
                self.request.user, "Public assessments"
