...
class MetaProtocol(models.Model):
    ...
    protocol_type = models.PositiveSmallIntegerField(
        choices=constants.MetaProtocol.choices, default=0
    )
...
class MetaResult(models.Model):
    ...
    def get_docx_template_context(assessment, queryset):
        ...
        for thisMr in results:
            ...
            if mr is None:
                mr = thisMr
                mr["ci_percent"] = int(mr["ci_units"] * 100.0)
                mr["adjustments_list"] = ", ".join(sorted(mr["adjustment_factors"]))
        ...
        for study in studies:
            study["protocols"] = sorted(
                list(study["protocols"].values()), key=lambda obj: (obj["name"].lower())
            )
            for pro in study["protocols"]:
                ...
                for obj in pro["statistical_methods"]:
                    obj["sm_endpoints"] = "; ".join([d["label"] for d in obj["sm_endpoints"]])
