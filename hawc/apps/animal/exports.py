...
import numpy as np
import pandas as pd
from django.db.models import Exists, F, OuterRef, CharField
...
class EndpointGroupExport(ModelExport):
    ...
    def get_annotation_map(self, query_prefix):
        return {
            "treatment_effect_display": sql_display(query_prefix + "treatment_effect", constants.TreatmentEffect, default=None),
...
class EndpointGroupFlatDataPivot2(FlatFileExporter):
    ...
    def handle_percent_control(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df.groupby("endpoint-id", group_keys=False, sort=False).apply(_func).reset_index(drop=True)
    ...
    def handle_dose_groups(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def _func(group_df: pd.DataFrame) -> pd.Series:
            ...
            NOEL_series = group_df["dose_group-dose"][group_df["endpoint_group-NOEL"].fillna(False)&pd.notna(reported_doses)]
            group_df[noel_names.noel] = NOEL_series.iloc[0] if NOEL_series.size > 0 else None
            LOEL_series = group_df["dose_group-dose"][group_df["endpoint_group-LOEL"].fillna(False)&pd.notna(reported_doses)]
            group_df[noel_names.loel] = LOEL_series.iloc[0] if LOEL_series.size > 0 else None
            FEL_series = group_df["dose_group-dose"][group_df["endpoint_group-FEL"].fillna(False)&pd.notna(reported_doses)]
...
class EndpointSummary2(EndpointGroupFlatDataPivot2):
    def _set_responses(self, df: pd.DataFrame):
        def _func(group_df: pd.DataFrame) -> pd.Series:
            ...
            group_df["responses"] = [response_or_incidence_with_variance.reset_index(drop=True)] * group_df.shape[0]
    ...
    def _set_ns(self, df: pd.DataFrame):
        def _func(group_df: pd.DataFrame) -> pd.Series:
            unique_df = group_df.drop_duplicates(subset="endpoint_group-id")
            group_df["ns"] = [
                unique_df["endpoint_group-n"].map("{:g}".format, na_action="ignore").fillna("").reset_index(drop=True)
            ] * group_df.shape[0]
    ...
    def _set_doses(self, df: pd.DataFrame):
        def _func(group_df: pd.DataFrame) -> pd.Series:
            def __func(_group_df: pd.DataFrame) -> pd.Series:
                _group_df["doses"] = [
                    _group_df["dose_group-dose"].map("{:g}".format, na_action="ignore").fillna("").reset_index(drop=True)
                ] * _group_df.shape[0]
                return _group_df
