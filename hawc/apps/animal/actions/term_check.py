...
class EhvTermCheckSchema(BaseModel):
    assessment_id: int


class EhvTermCheck(BaseApiAction):
    """
    Generate a report for potential issues identified in a given assessment and the current
    EHV vocabulary.
    """

    input_model = EhvTermCheckSchema
    ...
    def evaluate(self):
        ...
        for endpoint in Endpoint.objects.filter(
            assessment_id=self.inputs.assessment_id
