...
class AnimalAssessmentViewSet(viewsets.GenericViewSet):
    ...
    def study_heatmap(self, request, pk):
        ...
        df = cacheable(
            lambda: models.Endpoint.heatmap_study_df(
    ...
    def endpoint_heatmap(self, request, pk):
        ...
        df = cacheable(
            lambda: models.Endpoint.heatmap_df(self.assessment.id, published_only=not unpublished),
            key,
        )
    ...
    def endpoint_doses_heatmap(self, request, pk):
        ...
        df = cacheable(
            lambda: models.Endpoint.heatmap_doses_df(
                self.assessment, published_only=not unpublished
            ),
            key,
        )
    ...
    def endpoints(self, request, pk):
        ...
        df = cacheable(
            lambda: models.Endpoint.objects.endpoint_df(
                self.assessment, published_only=not unpublished
            ),
            key,
        )
