...
yl.registerFunction = function (name, func) {
  if (this.functions.hasOwnProperty(name)) {
    ...
    console.error(
      'The DAL function "' + name + '" has already been registered.'
    );
    return;
  }
  ...
  var event = new CustomEvent("dal-function-registered." + name, {
    detail: { name: name, func: func },
  });
...
(function ($, yl) {
  ...
  yl.getForwards = function (element) {
    ...
    divSelector =
      "div.dal-forward-conf#dal-forward-conf-for-" +
      element.attr("id") +
      ", " +
