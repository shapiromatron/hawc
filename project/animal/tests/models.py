...
class AnimalGroup(TestCase):
    ...
    def test_clean(self):
        ...
        self.assertItemsEqual(
            err.exception.messages, ["Error- selected strain is not of the selected species."],
        )
    ...
    def oldtest_get_doses_json(self):
        self.assertEqual(
            '[{"error": "no dosing regime"}]', self.animal_group_working.get_doses_json(),
        )

        self.dosing_regime_working = models.DosingRegime(
            dosed_animals=self.animal_group_working, route_of_exposure="I", description="foo",
        )
        ...
        self.assertEqual(
            '[{{"name": "mg/kg/day", "values": [0.0, 50.0], "id_": {0}}}]'.format(
                str(self.dose_units.pk)
            ),
...
class DosingRegime(TestCase):
    ...
    def test_get_doses_json(self):
        ...
        self.assertItemsEqual(v[0]["values"], [0.0, 50.0])
        self.assertEqual(v[1]["name"], "mg/L")
        self.assertItemsEqual(v[1]["values"], [100, 150.0])
...
class EndpointGroupPercentControl(TestCase):
    def _build_egdict(self, egs):
        output = []
        for eg in egs:
            output.append({"n": eg[1], "response": eg[2], "stdev": eg[3]})
        return output
