...
let buildFilterTable = function(tab, dp, handleTableChange){
        var thead = $('<thead>').html(buildHeaderTr([
                'Field name', 'Filter type', 'Value', 'Ordering'])),
            tbody = $('<tbody>')
                .on('change autocompletechange', 'input,select', handleTableChange),
    ...
    buildSortingTable = function(tab, dp, handleTableChange){
        let thead = $('<thead>').html(buildHeaderTr([
                'Field name', 'Sort order', 'Ordering'])),
            tbody = $('<tbody>')
    ...
    buildSpacingTable = function(tab, dp){
        let tbody = $('<tbody>'),
            thead = $('<thead>').html(buildHeaderTr([
                'Row index', 'Show line?', 'Line style',
                'Extra space?', 'Delete'])),
    ...
    buildManualOverrideRows = function(dp, tbody){
        ...
        data.forEach(function(v){
            let obj = get_matched_override_or_default(v._dp_pk),
                ...
                include = $('<input name="ov_include" type="checkbox">')
                    .prop('checked', obj.include),
                index = $('<input name="ov_index" class="span12" type="number" step="any">')
                    .val(obj.index),
                text_style = dp.style_manager
                    .add_select('texts', obj.text_style, true),
                line_style = dp.style_manager
                    .add_select('lines', obj.line_style, true),
                symbol_style = dp.style_manager
                    .add_select('symbols', obj.symbol_style, true);
    ...
    buildOrderingTable = function(tab, dp, tbody){
        let thead = $('<thead>').html(buildHeaderTr([
                'Description', 'Include', 'Row index',
                'Override text style', 'Override line style',  'Override symbol style',
            ])),
            ...
            refreshRowsBtn = $('<button class="btn btn-info" style="margin-left: 2em"><i class="fa fa-refresh"></i> Refresh</button>')
                    .on('click', () => buildManualOverrideRows(dp, tbody)),
            resetOverridesBtn = $('<button class="btn btn-danger pull-right"><i class="fa fa-trash"></i> Reset</button>')
                    .on('click', resetOverrideRows),
            tbl = $('<table class="table table-condensed table-bordered table-hover tbl_override">')
                .html([thead, tbody]);
    ...
    showOverrideRebuildRequired = function(dp, tbody){
        let btn = $('<button class="btn btn-primary">Click to rebuild</button>')
                    .on('click', function(){
                        buildManualOverrideRows(dp, tbody);
                    }),
            td = $('<td colspan="6">')
                    .append('<p>Row-ordering has changed.</p>', btn);
