...
class _DataPivot_settings_refline {
    ...
    data_push() {
        ...
        this.values.line_style = this.content.line_style
...
class _DataPivot_settings_refrect {
    ...
    data_push() {
        ...
        this.values.rectangle_style = this.content.rectangle_style
            .find('option:selected')
            .text();
    }
}
...
class _DataPivot_settings_label {
    constructor(data_pivot, values) {
        ...
        // create fields
        this.content = {};
        this.content.text = $('<input class="span12" type="text">').val(
            values.text
        );
        this.content.style = this.data_pivot.style_manager.add_select(
            'texts',
            values.style
        );

        var movement_td = DataPivot.build_movement_td(
            self.data_pivot.settings.labels,
            this,
            { showSort: false }
        );
...
class _DataPivot_settings_sorts {
    constructor(data_pivot, values, index) {
        var self = this,
            movement_td = DataPivot.build_movement_td(
                data_pivot.settings.sorts,
                this,
                { showSort: true }
            );
        this.data_pivot = data_pivot;
        ...
        this.content.field_name
            ...
            .prop('selected', true);
        this.content.ascending
            .find('[value={0}]'.printf(values.ascending))
            .prop('checked', true);
    ...
    data_push() {
        this.values.field_name = this.content.field_name
            .find('option:selected')
            .text();
        this.values.ascending = this.content.ascending
            .find('input')
            .prop('checked');
    }
}
...
class _DataPivot_settings_filters {
    constructor(data_pivot, values) {
        ...
        ).autocomplete({ source: values.value });
        ...
        var movement_td = DataPivot.build_movement_td(
            self.data_pivot.settings.filters,
            this,
            { showSort: true }
        );
        ...
        this.tr = $('<tr>')
            ...
            .append(movement_td)
            .on(
                'change autocompletechange autocompleteselect',
                'input,select',
                function(v) {
                    self.data_push();
                }
    ...
    data_push() {
        this.values.field_name = this.content.field_name
            .find('option:selected')
            .val();
        this.values.quantifier = this.content.quantifier
            .find('option:selected')
            .val();
...
class _DataPivot_settings_spacers {
    constructor(data_pivot, values, index) {
        ...
        this.content = {
            ...
            line_style: data_pivot.style_manager.add_select(
                'lines',
                values.line_style
            ),
...
class _DataPivot_settings_description {
    constructor(data_pivot, values) {
        ...
        this.content = {
            ...
            header_style: this.data_pivot.style_manager.add_select(
                'texts',
                values.header_style
            ),
            text_style: this.data_pivot.style_manager.add_select(
                'texts',
                values.text_style
            ),
            max_width: $('<input class="span12" type="number">'),
            dpe: $('<select class="span12"></select>').html(
                this.data_pivot.dpe_options
            ),
        };
        ...
        this.content.dpe
            .find('option[value="{0}"]'.printf(values.dpe))
            .prop('selected', true);
    ...
    data_push() {
        this.values.field_name = this.content.field_name
            .find('option:selected')
            .val();
        this.values.field_index = this.content.field_name
            .find('option:selected')
            .val();
        this.values.header_style = this.content.header_style
            .find('option:selected')
            .val();
        this.values.text_style = this.content.text_style
            .find('option:selected')
            .val();
        this.values.header_name = this.content.header_name.val();
        this.values.max_width =
            parseFloat(this.content.max_width.val(), 10) || undefined;
...
class _DataPivot_settings_pointdata {
    constructor(data_pivot, values) {
        ...
        this.content = {
            ...
            header_name: $('<input class="span12" type="text">'),
            marker_style: this.data_pivot.style_manager.add_select(
                style_type,
                values.marker_style
            ),
        ...
        this.tr = $('<tr>')
            ...
            .on('change', 'input,select', function(v) {
                ...
                self.data_pivot.legend.add_or_update_field({
                    symbol_index: self.data_pivot.settings.datapoint_settings.indexOf(
                        values
                    ),
                    label: self.content.header_name.val(),
                    symbol_style: self.content.marker_style
                        .find('option:selected')
                        .text(),
                });
            });
    ...
    data_push() {
        this.values.field_name = this.content.field_name
            ...
            .val();
        this.values.header_name = this.content.header_name.val();
        this.values.marker_style = this.content.marker_style
            .find('option:selected')
            .text();
        this.values.dpe = NULL_CASE;
...
class _DataPivot_settings_linedata {
        ...
        this.tr = $('<tr>')
            ...
            .on('change', 'input,select', function(v) {
                ...
                var obj = {
                    line_index: index,
                    label: self.content.header_name.val(),
                    line_style: self.content.marker_style
                        .find('option:selected')
                        .text(),
                };
    ...
    data_push() {
        var v = {
            low_field_name: this.content.low_field_name
                .find('option:selected')
                .val(),
            high_field_name: this.content.high_field_name
                .find('option:selected')
                .val(),
            header_name: this.content.header_name.val(),
            marker_style: this.content.marker_style
                .find('option:selected')
                .text(),
        };
...
class _DataPivot_settings_general {
    constructor(data_pivot, values) {
        ...
        this.content = {
            ...
            show_xticks: $('<input type="checkbox">').prop(
                'checked',
                values.show_xticks
            ),
            show_yticks: $('<input type="checkbox">').prop(
                'checked',
                values.show_yticks
            ),
            ...
            logscale: $('<input type="checkbox">').prop(
                'checked',
                values.logscale
            ),
            domain: $(
            ...
            merge_aggressive: $('<input type="checkbox">').prop(
                'checked',
                values.merge_aggressive
            ),
            merge_until: $('<select name="merge_until">'),
            text_background: $('<input type="checkbox">').prop(
                'checked',
                values.text_background
            ),
            text_background_color: $('<input type="color">').val(
        ...
        this.trs = [
            ...
            build_tr(
                'Axis minimum and maximum<br>(ex: "1,100")',
                this.content.domain
            ),
            ...
            build_tr(
                'Highlight background text color',
    ...
    data_push() {
        ...
        this.values.minimum_row_height = parseInt(
            this.content.minimum_row_height.val(),
            10
        );
        this.values.font_style = this.content.font_style
            .find('option:selected')
            .val();
        ...
        this.values.padding.right = parseInt(
            this.content.padding_right.val(),
            10
        );
        this.values.padding.bottom = parseInt(
            this.content.padding_bottom.val(),
            10
        );
        this.values.padding.left = parseInt(
            this.content.padding_left.val(),
            10
        );
        this.values.merge_descriptions = this.content.merge_descriptions.prop(
            'checked'
        );
        this.values.merge_aggressive = this.content.merge_aggressive.prop(
            'checked'
        );
        this.values.merge_until =
            parseInt(this.content.merge_until.val(), 10) || 0;
        this.values.text_background = this.content.text_background.prop(
            'checked'
        );
...
class _DataPivot_settings_barchart {
    constructor(dp) {
        let values = dp.settings.barchart,
            ...
            styleSelectFactory = dp.style_manager.add_select.bind(
                dp.style_manager
            ),
            content = {
                field_name: $(
                    '<select id="bc_field_name" name="field_name" class="span12">'
                )
                ...
                error_marker_style: styleSelectFactory(
                    'lines',
                    values.error_marker_style
                ),
        ...
        div
            .find('label[for="bc_error_show_tails"]')
