...
class InlineRendering {
    ...
    render(){
        this.renderParent = $('<div class="inlineSmartTagParent">')
            .insertBefore(this.$div);

        let $title = $('<div class="row-fluid inlineSmartTagTitle">'),
            $body = $('<div class="row-fluid inlineSmartTagBody">'),
            $caption = $('<div class="row-fluid inlineSmartTagCaption">').append(this.$div.detach()),
            $container = $('<div class="inlineSmartTagContainer container-fluid">').append([$title, $body, $caption]);
    ...
    setTitle($titleContent){
        let $title = this.renderParent.find('.inlineSmartTagTitle'),
            ...
            expandHideTogger = function(){
                ...
                if (isMax){
                    $title.find('.icon-minus').removeClass('icon-minus').addClass('icon-plus');
                    $body.fadeOut('slow');
                } else {
                    $title.find('.icon-plus').removeClass('icon-plus').addClass('icon-minus');
                    $body.fadeIn('slow');
                }
            },
