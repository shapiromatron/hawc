...
class CrossviewPlot extends D3Visualization {
    ...
    processData(){
        ...
        var filters_map = d3.map({
            lt(val, tgt){return val<tgt;},
            lte(val, tgt){return val<=tgt;},
            gt(val, tgt){return val>tgt;},
            gte(val, tgt){return val>=tgt;},
            contains(val, tgt){
...
CrossviewPlot._cw_filter_process = {
    study(d){return d.data.animal_group.experiment.study.short_citation; },
    experiment_type(d){return d.data.animal_group.experiment.type; },
    route_of_exposure(d){return d.data.animal_group.dosing_regime.route_of_exposure; },
    lifestage_exposed(d){return d.data.animal_group.lifestage_exposed; },
    species(d){return d.data.animal_group.species; },
    sex(d){return d.data.animal_group.sex; },
    generation(d){return d.data.animal_group.generation; },
    effects(d){return d.data.effects.map(function(v){return v.name; });},
    system(d){return d.data.system; },
    organ(d){return d.data.organ; },
    effect(d){return d.data.effect; },
