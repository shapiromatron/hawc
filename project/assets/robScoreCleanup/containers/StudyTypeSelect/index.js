...
import { selectStudyType } from 'robScoreCleanup/actions/StudyTypes';
    ...
    render() {
        return (
            <div>
                ...
                <select multiple name="studyType_filter" id="studyType_filter" onChange={this.handleChange} style={{height: '120px'}}>
                {_.map(this.props.choices, (type, i) => {
                    return <option key={i} value={type}>{h.caseToWords(type)}</option>;
                })}
                </select>
            </div>
        );
