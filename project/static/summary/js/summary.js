...
SummaryTextTree.prototype.get_summaries = function(){
  var self = this,
      url = '/summary/assessment/{0}/summaries/json/'.printf(this.options.assessment_id);
  $.get(url, function(d){
    self.root = new SummaryText(d[0], 1, self, 0);
    if((self.root.data.comments) && (self.root.data.comments.length>0)){self._unpack_comments();}
...
SummaryTextTree.prototype._update_read = function(){
...
SummaryTextTree.prototype._unpack_comments = function(){
    var self = this;
...
SummaryTextTree.prototype._update_modified = function(){
...
SummaryTextTree.prototype.render_docheaders = function(){
    var contents = [];
...
SummaryTextTree.prototype.render_doctext = function(){
    var contents = [];
    if (this.root.children.length===0){
...
SummaryTextTree.prototype.update_textdiv = function(obj){
...
SummaryTextTree.prototype._show_move_node_options = function(show){
    if(show){
...
SummaryTextTree.prototype.set_modify_events = function(){
    var self = this;
...
SummaryTextTree.prototype.render_doctree = function(){
    var contents=[];
    if (this.root.children.length===0){
...
SummaryTextTree.prototype.get_summarytext_node = function(id){
    var node,
...
SummaryTextTree.prototype.submit = function(){
    var self = this,
...
SummaryTextTree.prototype.enable_affix = function(){
...
SummaryText.prototype.get_option_item = function(lst){
...
SummaryText.prototype.get_children_option_items = function(lst){
    this.children.forEach(function(v){
        lst.push('<option value="{0}">{1}</option>'.printf(v.id, v.data.title));
    });
};

SummaryText.prototype.render_tree = function(lst){
...
SummaryText.prototype.get_tab_depth = function(){
    return Array(this.depth-1).join('&nbsp;&nbsp;');
};

SummaryText.prototype.print_section = function(){
...
SummaryText.prototype.render_header = function(lst){
...
SummaryText.prototype.render_body = function(lst){
    lst.push(this.print_section());
    this.children.forEach(function(v){v.render_body(lst);});
};

SummaryText.prototype.get_prior_sibling_id = function(){
    var self=this,
        result = -1;
    if(this.parent){
...
SummaryText.prototype._add_comment = function(comment_data){
