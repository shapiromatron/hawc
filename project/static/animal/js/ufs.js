/*
 * TODO:
 * - add popup to view endpoint
 * - add ability to globally change descriptions
 */


UncertaintyFactor = function(parsed_json, endpoint, changed){
...
UncertaintyFactor.prototype.build_submission_object = function(endpoint_pk){
...
UncertaintyFactor.prototype.build_form = function(){
...
UncertaintyFactor.prototype.change = function(obj){
    this.changed=true;
    this.value = obj.val;
    this.input_field.val(obj.val);
};

UncertaintyFactor.prototype.build_popover = function(){
...
UncertaintyFactor.prototype.set_description = function(txt){
...
UncertaintyFactor.prototype.build_table_row = function(){
...
UFDescription.prototype.hide = function(){
    this.div.fadeOut('fast');
    this.uf.set_description(this.input.val());
    this.uf = undefined;
};

UFDescription.prototype.set_UF = function(uf){
    this.uf = uf;
...
UFsContainer =  function(endpoints, name, options){
...
UFsContainer.prototype = new Aggregation();
UFsContainer.prototype.constructor = Aggregation;

UFsContainer.prototype.update_domain = function(){
...
UFsContainer.prototype.get_global_domain = function(){
    return {domain: this.x_domain, changed: this.domain_changed};
};

UFsContainer.prototype.update = function(){
    this.update_domain();
    this.redraw_all_plots();
};

UFsContainer.prototype.build_all_plots = function(){
...
UFsContainer.prototype.redraw_all_plots=function(first_time){
...
UFsContainer.prototype.submit_data = function(){
...
UFsContainer.prototype.update_all_ufs = function(opt){
...
UFsContainer.prototype.build_change_all_fields = function(){
...
UFsContainerTable = function(ufs_container, table_id){
    this.ufs_container = ufs_container;
    this.table = $(table_id);
    this.build_table();
};

UFsContainerTable.prototype.build_header_row = function(){
    var tr = $('<tr></tr>');
...
UFsContainerTable.prototype.build_table = function(){
...
ReferenceValue = function(dataset, options){
    this.observers = [];
    this.data = dataset;
...
ReferenceValue.prototype.unpack_endpoint_ufs = function(){
    UFEndpoint.prototype.unpack_endpoint_ufs.apply(this);
};

ReferenceValue.prototype.create_plot_dataset = function(){
...
ReferenceValue.prototype.calculate_plot_values = function(){
    var threes = 0, last_v;
    this.data.ufs.forEach(function(v, i){
        if (v.value === 3){
            threes += 1;
...
UFEndpoint = function(endpoint, options){
...
UFEndpoint.prototype = new Endpoint();
UFEndpoint.prototype.constructor = Endpoint;

UFEndpoint.prototype.unpack_endpoint_ufs = function(){
...
UFEndpoint.prototype.build_uf_plot = function(){
    var options = {build_plot_startup: true,
                   default_x_axis: 'log',
                   show_legend: false,
                   show_title: false,
...
UFEndpoint.prototype.create_uf_form = function(){
...
UFEndpoint.prototype.set_UF_container = function(container){
    this.ufs_container = container;
};

UFEndpoint.prototype.print_pod_rfd = function(){
...
UFEndpoint.prototype.create_plot_dataset = function(){
    var plot_data = {pk: this.data.pk,
                     pod: this.pod.value,
                     rfd: this.rfd,
                     name: this.data.study.short_citation + "- " +
                           this.data.animal_group + ": " + this.data.name,
...
UFEndpoint.prototype.calculate_rfd = function(){
...
UFEndpoint.prototype.build_table_row = function(){
    var tr = $('<tr></tr>');
    tr.append($('<td></td>').html('<a href="{0}">{1}</a>'
              .printf(this.data.study.study_url, this.data.study.short_citation)));
    tr.append($('<td></td>').html('<a href="{0}">{1}</a>'
              .printf(this.data.experiment_url, this.data.experiment)));
    tr.append($('<td></td>').html('<a href="{0}">{1}</a>'
              .printf(this.data.animal_group_url, this.data.animal_group)));
    tr.append($('<td></td>').html('<a href="{0}">{1}</a>'
...
UFEndpoint.prototype.uf_changed = function(obj){
...
UFEndpoint.prototype.build_submission_object = function(){
...
UncertaintyFactorPlot = function(objects, plot_div, options){
...
UncertaintyFactorPlot.prototype = new D3Plot();
UncertaintyFactorPlot.prototype.constructor = UncertaintyFactorPlot;

UncertaintyFactorPlot.prototype.update = function(){
    this.update_plot();
};

UncertaintyFactorPlot.prototype.update_plot = function(){
...
UncertaintyFactorPlot.prototype.build_plot = function(){
    this.plot_div.html('');
    this.get_plot_sizes();
    this.get_dataset();
...
UncertaintyFactorPlot.prototype.resize_plot_width = function(){
...
UncertaintyFactorPlot.prototype.x_axis_change_chart_update = function(domain_changed){
...
UncertaintyFactorPlot.prototype.build_text_labels = function(){
    var y = this.y_scale,
        x = this.x_scale,
...
UncertaintyFactorPlot.prototype.get_plot_sizes = function(){
...
UncertaintyFactorPlot.prototype.set_defaults = function(){
...
UncertaintyFactorPlot.prototype.get_dataset = function(){
    ...
    if (this.objects[0].constructor===ReferenceValue){
        ...
        this.objects.forEach(function(v,i){
            if (v.data.dr.length>0) dataset.push(v.create_plot_dataset());
        });
...
UncertaintyFactorPlot.prototype.add_axes = function() {
...
UncertaintyFactorPlot.prototype.add_critical_points = function(){
...
UncertaintyFactorPlot.prototype.show_point_labels = function(plot, class_name){
    bars = plot.vis.selectAll('.data_label.' + class_name)
                    .classed('hidden', false);
};

UncertaintyFactorPlot.prototype.hide_point_values = function(plot){
    bars = plot.vis.selectAll('.data_label')
                    .classed('hidden', true);
};

UncertaintyFactorPlot.prototype.add_uf_rectangles = function(){
    var x = this.x_scale,
        y = this.y_scale,
        uf_order = this.uf_order,
...
UncertaintyFactorPlot.prototype.show_rectangle_values = function(bar, plot){
    bar = d3.select(bar);
...
UncertaintyFactorPlot.prototype.hide_rectangle_values = function(bar, plot){
    plot.bars
        .classed('unhovered_bar', false)
...
UncertaintyFactorPlot.prototype.add_legend = function(){
    var items = [],
        self = this;
