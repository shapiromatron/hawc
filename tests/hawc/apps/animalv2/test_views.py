...
from ..test_utils import check_200, check_403, generic_get_any, get_client


@pytest.mark.django_db
class TestExperimentView:
    def test_permission(self, db_keys):
        experiment = generic_get_any(models.Experiment)
        url = reverse("animalv2:experiment_detail", args=(experiment.id,))
        client = get_client()
        check_403(client, url)

    def test_success(self, db_keys):
        random_experiment = generic_get_any(models.Experiment)
