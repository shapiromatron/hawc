...
class TestCreatePermissions:
    def test_success(self, db_keys):
        # with authentication
        url = reverse("epiv2:des_create", kwargs={"pk": db_keys.study_working})
        c = Client()
...
class TestDetailPermissions:
    def test_detail(self, db_keys):
        ...
        for user in users:
            ...
            response = c.get(reverse("epiv2:des_detail", kwargs={"pk": db_keys.epiv2_design}))
            assert response.status_code == 200

        # anon user should not view
        c = Client()
        url = reverse("epiv2:des_detail", kwargs={"pk": db_keys.epiv2_design})
        response = c.get(url)
