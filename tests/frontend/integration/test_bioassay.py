...
class TestBioassay(PlaywrightTestCase):
    def test_bioassay(self):
        ...
        expect(
            page.locator('css=#objContainer table >> has-text("2 year bioassay")')
        ).to_have_count(1)
        ...
        page.expect(page.locator("text=my outcome")).to_be_visible()
        expect(page.locator("css=svg")).to_be_visible()
        expect(page.locator("css=.d3 .dr_dots .dose_points")).to_have_count(3)
        expect(page.locator("#dr-tbl")).to_be_visible()
