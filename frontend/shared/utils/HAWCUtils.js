...
class HAWCUtils {
    ...
    static resetDynamicForm(formSelector, values, errors) {
        ...
        root.find("#udf-form .is-invalid").removeClass("is-invalid");
        root.find("#udf-form .bg-pink").removeClass("bg-pink");
        _.forEach(_.fromPairs(errors), function(error, field) {
            var input = root.find(`[name="${field}"]`);
