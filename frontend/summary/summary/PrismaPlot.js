...
class PrismaPlot {
    ...
    build_plot() {
        ...
        function makeGroupTextVisible(node) {
        ...
        function resizeNodes(node) {
            ...
            if (node.isVertical && !node.parent) {
                ...
                updateNodeTextAttributes(node, { "y": prevNodeGroup.y + prevNodeGroup.height + nodeSpacing + TEXT_OFFSET_Y });
            }
            ...
            for (let child of node.children) {
                ...
                if (child.isVertical) {
                    ...
                    updateNodeTextAttributes(child, { "y": prevChildBBox.y + prevChildBBox.height + prevChildNodeSpacing + TEXT_OFFSET_Y });
                }
                else if (child.previous) {
                    let prevBBox = child.previous.rect.getBBox();
                    child.rect.setAttribute("y", prevBBox.y);
                    updateNodeTextAttributes(child, { "y": prevBBox.y + TEXT_OFFSET_Y });
                }
                else {
                    let prevChildBBox = node.text[node.text.length - 1].getBBox();
                    child.rect.setAttribute("y", prevChildBBox.y + prevChildBBox.height + 15);
                    updateNodeTextAttributes(child, { "y": prevChildBBox.y + prevChildBBox.height + TEXT_OFFSET_Y + 15 });
                }
                resizeNodes(child);
            }
        ...
        function updateNodeTextAttributes(node, attr = {}) {
            node = getGroup(node.id);
        ...
        function createRectangleWithText(id = "", text = "", x = 0, y = 0, styling = {}) {
        ...
        function addTextToNode(node, id, text, styling = {}) {
            for (const [key, value] of Object.entries(STYLE_DEFAULT)) {
                if (!styling[key]) styling[key] = value;
            }
        ...
        function addNodeToHTML(node, parent = undefined) {
            if (parent) {
            ...
            let txtEle = addTextToNode(node, node.id + "-text", node.text, node.styling);
            node.text = txtEle;
        ...
        function drawConnection(id1, id2, styling = {}) {
            let id = id1 + id2;
            let node1BBox = getGroup(id1).rect.getBBox();
            let node2BBox = getGroup(id2).rect.getBBox();
            ...
            if (node1BBox.x == node2BBox.x || styling["arrow-force-vertical"] == "true") {
                ...
                connectPoints(id + "_1", node1XY, node1Midpoint, false, styling);

                let node2Midpoint = [
                    node2BBox.x + node2BBox.width / 2,
                    yMidpoint
                ];
                connectPoints(id + "_2", node1Midpoint, node2Midpoint, false, styling);

                let node2XY = [
                    node2BBox.x + node2BBox.width / 2,
                    node2BBox.y
                ];
                connectPoints(id + "_3", node2Midpoint, node2XY, true, styling);
            }
            else {
        ...
        function connectPoints(id, xy1, xy2, arrowhead = true, styling = {}) {
            for (const [key, value] of Object.entries(STYLE_DEFAULT)) {
                if (!styling[key]) styling[key] = value;
            }
        ...
        function initNode(id, text, styling = {}) {
            let initialNode = createRectangleWithText(
                id,
                text,
                WORKSPACE_START_X,
                WORKSPACE_START_Y,
            ...
            return addNodeToHTML(initialNode);
        }

        function createNewVerticalNode(prevNode, id, text, group = false, styling = {}) {
            prevNode = getGroup(prevNode.id);
            let prevBBox = prevNode.rect.getBBox();
            let nodeSpacing = styling["spacing-vertical"] ? parseFloat(styling["spacing-vertical"]) : SPACING_V;
            let y = prevBBox.y + prevBBox.height + nodeSpacing;

            let verticalNode = createRectangleWithText(id, text, prevBBox.x, y, styling);
            verticalNode.group.setAttribute("previous", prevNode.id);
            verticalNode.group.setAttribute("is-vertical", true);

            if (group) {
                return addNodeToHTML(verticalNode, prevNode.parent);
            }
            return addNodeToHTML(verticalNode);
        }

        // group nodes if prevNode has a parent
        function createNewHorizontalNode(prevNode, id, text, group = false, styling = {}) {
            prevNode = getGroup(prevNode.id);
            let prevBBox = prevNode.rect.getBBox();
            let x;
            let nodeSpacing = styling["spacing-horizontal"] ? parseFloat(styling["spacing-horizontal"]) : SPACING_H;
            ...
            let horizontalNode = createRectangleWithText(id, text, x, prevBBox.y, styling);
            horizontalNode.group.setAttribute("previous", prevNode.id);
            horizontalNode = addNodeToHTML(horizontalNode, prevNode.parent);
        ...
        function createChildNode(parent, id, text, styling = {}) {
            let pad = 15;
            let parentBBox = parent.rect.getBBox();

            let childAdjustedY = parentBBox.y + pad;
            let childAdjustedX = parentBBox.x + pad;
            let child = createRectangleWithText(id, text, childAdjustedX, childAdjustedY, styling);
            ...
            return addNodeToHTML(child, parent);
        }
        ...
        var cardRowLength = -1;
        function createCard(parent, text, styling = {}) {
            ...
            if (allCards.length > 0) {
                ...
                if (cardPosition < parentPosition) {
                    return createNewHorizontalNode(prevNode, id, text, true, styling);
                }
                else { // new row
                    prevNode = allCards[allCards.length - cardRowLength];
                    cardRowLength = 0;
                    return createNewVerticalNode(prevNode, id, text, true, styling);
                }
            }
            cardRowLength = 0;
            return createChildNode(parent, id, text, styling);
        }

        // helper function for parsing the data structure
        // handles list/card layouts at the section and block level
        function listCardLayout(id, col) {
            ...
            if (col.block_layout == "list") {
                for (let i = 0; i < blocks.length; i++) {
                    ...
                    let txtEle = addTextToNode(currentNode, subblockId, subblockText, blockStyle);
                ...
                for (let subblock of blocks) {
                    ...
                    createCard(currentNode, subblock.label, blockStyle);
                }
            }
        }
        ...
        window.onload = function () {
            ...
            for (let row of diagramSections) {
                ...
                if (!parent) {
                    parent = initNode(blockId, blockBoxInfo, sectionStyle);
                }
                else {
                    parent = createNewVerticalNode(parent, blockId, blockBoxInfo, false, sectionStyle);
                }

                if (row.block_layout) {
                    listCardLayout(blockId, row);
                    resizeNodes(parent);
                    continue;
                }
                ...
                for (let i = 0; i < row.blocks.length; i++) {
                    ...
                    if (i > 0) {
                        ...
                        sibling = createNewHorizontalNode(previous, id, boxInfo, true, blockStyle);
                    }
                    else {
                        child = createChildNode(parent, id, boxInfo, blockStyle);
                        sibling = undefined;
                    }
            ...
            for (let connection of connections) {
                ...
                drawConnection(srcId, dstId, connection.styling);
            }
        }
    }
}
