...
General commands for building the R package:
...
.. code-block:: bash
    ...
    R -e "plumber::pr_run(plumber::pr('r/hawc/R/plumber.R'), port = 8010)"

Swagger API documentation should be available at http://127.0.0.1:8010/__docs__/.
...
.. code-block:: bash
    ...
    # healthcheck
    curl -s http://127.0.0.1:8010/api/v1/healthcheck | json_pp

    # JSON data in/out
    curl -s --header "Content-Type: application/json" \
        --request POST \
        --data '{"a": 10, "b": 100}' \
        http://127.0.0.1:8010/api/v1/sum | json_pp

To deploy the R server:
...
.. code-block:: bash
    ...
    curl -s http://127.0.0.1:5010/api/v1/healthcheck | json_pp

    curl -s --header "Content-Type: application/json" \
        --request POST \
        --data '{"a": 10, "b": 100}' \
        http://127.0.0.1:5010/api/v1/sum | json_pp
